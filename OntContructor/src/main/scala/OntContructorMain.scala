...
object OntContructorMain {
  def main(args: Array[String]): Unit = {
    ...
    val ontology = manager.createOntology(IRI.create(ONTOLOGYURI,"conference#"))
    //Prefix for all the entities
    val pm = new DefaultPrefixManager(null, null, ONTOLOGYURI+"conference#")


    // Declaration Axiom for creating Classes
    val classes=Source.fromFile("data/confClass").getLines()
    ...
    val objprop=Source.fromFile("data/confObjectProperties").getLines()
    objprop.foreach(f=> {
      val farr=f.split(",")
    ...
    val dataprop=Source.fromFile("data/confDataProperties").getLines()
    ...
    val individuals=Source.fromFile("data/confIndividuals").getLines()
    ...
    val triplets=Source.fromFile("data/confTriplets").getLines()
    triplets.foreach(f=>{
      val farr=f.split(",")
    ...
    val os = new FileOutputStream("data/conf.owl")
