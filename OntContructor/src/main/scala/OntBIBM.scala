...
object OntBIBM {
  def main(args: Array[String]): Unit = {
    ...
    val ontology = manager.createOntology(IRI.create(ONTOLOGYURI,"heart#"))
    //Prefix for all the entities
    val pm = new DefaultPrefixManager(null, null, ONTOLOGYURI+"heart#")
    ...
    val triplets=Source.fromFile("data/diabetes/checkTriplets").getLines()
    triplets.foreach(f=>{
      val farr=f.split(",")
    ...
    val os = new FileOutputStream("data/heart.owl")
